...
works from the LAN can be dead from outside it, and vice versa.
...
(`playbooks/utility/export-service-names.yaml`), then runs
...
redesign (#262). Re-run the script and diff against these files to prove a
change hasn't broken a working name, and that only the intended deltas
appear.
