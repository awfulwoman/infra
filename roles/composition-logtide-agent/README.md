...
**Exclusions.** `composition_logtide_agent_exclude_pattern` defaults to `^logtide-(backend|agent)`. Those two feed their own input back in if collected; the rest of the logtide stack is ingested normally, which is what you want when logtide itself is the thing misbehaving.
