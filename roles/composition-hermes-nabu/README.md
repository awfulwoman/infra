...
This is the "Nabu" persona - a single dedicated Hermes container, no Hermes
...
Its sibling is [`composition-hermes-jarvis`](../composition-hermes-jarvis) -
same role shape, different identity/model/MCP config. Not to be confused with
[`composition-nabu`](../composition-nabu), an unrelated Telegram "chives" bot
that already owns the `nabu` composition name and subdomain (same collision
on the `jarvis` side, with [`composition-jarvis`](../composition-jarvis) -
...
`composition_hermes_nabu_mcp_servers` is written verbatim under
...
`composition_hermes_nabu_mcp_env`, rendered into `.environment_vars` (`0600`,
`no_log`).

`composition_hermes_nabu_disabled_toolsets` is written to
...
`composition_hermes_nabu_matrix: true` connects the gateway to a Matrix
...
curl -s -X POST https://<homeserver>/_matrix/client/v3/login \
  -d '{"type":"m.login.password","identifier":{"type":"m.id.user","user":"nabu"},"password":"<account password>"}'
```

The response's `access_token` is `vault_hermes_nabu_matrix_access_token`.

## Key variables

| Variable | Default | Description |
|----------|---------|-------------|
| `composition_hermes_nabu_image` | `nousresearch/hermes-agent:latest` | Published image |
| `composition_hermes_nabu_model` | `{{ llm_default_model }}` | Model ID passed to Ollama |
| `composition_hermes_nabu_base_url` | `{{ llm_default_openai_uri }}` | Malcolm's OpenAI-compatible endpoint |
| `composition_hermes_nabu_context_length` | `64000` | Hermes' floor for agent use |
| `composition_hermes_nabu_terminal_backend` | `local` | Shell tool backend |
| `composition_hermes_nabu_mcp_servers` | `{}` | MCP servers for this bot |
...
| `composition_hermes_nabu_matrix_user_id` | `""` | e.g. `@nabu:matrix.ewwww.eu`; mandatory when on |
| `composition_hermes_nabu_matrix_allowed_users` | `[]` | Matrix IDs allowed to talk to the bot; env-only, no `config.yaml` equivalent |
| `composition_hermes_nabu_matrix_device_id` | `""` | Pins E2EE device identity - see above |
| `composition_hermes_nabu_matrix_require_mention` | `true` | Require `@mention` outside DMs |
| `composition_hermes_nabu_matrix_allowed_rooms` / `_free_response_rooms` | `[]` | Room IDs; joined into comma-separated `config.yaml` strings |
| `composition_hermes_nabu_dashboard_theme` / `_font` | `""` | Passed through to `config.yaml`'s `dashboard:` block |

## Vault variables

| Variable | Required | Description |
|----------|----------|--------------|
| `vault_hermes_nabu_dashboard_username` | when dashboard is on | Dashboard login (defaults to `vault_server_username`) |
| `vault_hermes_nabu_dashboard_password` | when dashboard is on | Dashboard password |
| `vault_hermes_nabu_dashboard_secret` | when dashboard is on | Signs session cookies; without it sessions die on restart |
| `vault_hermes_nabu_api_server_key` | when API server is on | Bearer key for the API endpoint |
| `vault_hermes_nabu_matrix_access_token` | when Matrix is on | Full account access - see Matrix section above |
...
provider wizard, say - set `composition_hermes_nabu_manage_config: false`.
Note that `hermes config set` routes API keys to `/opt/data/.env`, which
`.environment_vars` overrides.

## DNS

Registers subdomain: `hermes-nabu`
...
```bash
sudo zfs destroy -r <pool>/compositions/hermes-nabu
```
