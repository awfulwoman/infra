...
The role connects Open WebUI to an Ollama instance via `OLLAMA_BASE_URL`. By default this points to `llm_default_ollama_base_url` (the Mac Mini/Malcolm), where Ollama runs natively on macOS. Override `composition_open_webui_ollama_base_url` per-host if Ollama is running elsewhere.
