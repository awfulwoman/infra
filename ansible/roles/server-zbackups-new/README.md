...
- `low`: this dataset will enable daily snapshots and retain them for one week. The dataset and snapshots will be pulled to the central backup server.
- `critical`: this dataset will renable hourly snapshots and will retain them on a weekly, monthly, and year basis. The dataset and all snapshots will be pulled to the central backup server, and from there pushed to one or more offsite backup locations.
